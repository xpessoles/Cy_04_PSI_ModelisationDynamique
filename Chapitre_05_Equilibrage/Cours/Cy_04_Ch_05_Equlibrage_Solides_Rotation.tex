...
\begin{propriete}[Évolution des efforts au niveau des paliers au cours du temps]
...
\begin{itemize}
\item opérateur d'inertie : $E_3=0$ et $D_3=10^{-4}kg\cdot m^2$ ; 
\item Masse et centre de masse : $m_3=10kg$, $c=0mm$ et $b=0,05mm$ ; 
\item Rotation de $3/0$ : $\omega=30000tour/min$
\end{itemize}
...
Il apparaît des variations d'efforts de l'ordre de $10kN$ et de moment de $2kN\cdot m$ sur des périodes très brèves ($2ms$) ce qui constitue une source importante de vibrations pour la structure. Ce valeurs sont très supérieures à celles exigées par le cahier des charges puisque on ne tolère au maximum que $5N$.
\end{propriete}
...
\begin{exemple}[Mise en oeuvre de l'équilibrage statique du rotor $S_3$]
\subparagraph{}\textit{Déterminer les conditions d'équilibrage statique.}

%\begin{texteCache}
Les équations en résultantes impliquent que le paramètre $b$ doit être nul. Cela revient à dire que le centre d'inertie $G_3$ soit sur l'axe de rotation $\couple{O_3}{\overrightarrow{z}_{0,3}}$ de $S_3$ par rapport à $S_0$.
%\end{texteCache}
\end{exemple}
...
\begin{exemple}[Mise en oeuvre de l'équilibrage dynamique du rotor $S_3$]
\subparagraph{}\textit{Déterminer les conditions d'équilibrage dynamique.}

%\begin{texteCache}
Les équations en moment impliquent que $D_3=E_3=0$.
Cela revient à dire que l'axe $\couple{O_0}{\overrightarrow{z}_{0,3}}$ soit axe principal d'inertie du solide $S_3$.
%\end{texteCache}
\end{exemple}

\subsection{Conditions d'équilibrage}

\begin{definition}[Condition d'équilibrage d'un solide en rotation]
...
\subparagraph{}\textit{Traduire la relation donnée par l'équilibrage statique.}
\begin{itemize}
\item La condition d'équilibrage statique implique : $\overrightarrow{O_3G}\cdot \overrightarrow{x}_3=0$ et $\overrightarrow{O_3G}\cdot \overrightarrow{y}_3=0$.
\item La formule du barycentre donne : 
%\begin{texteCache}
$$
\overrightarrow{O_3G}=\frac{m_1\cdot \overrightarrow{O_3G_1}+m_1\cdot \overrightarrow{O_3G_2}+m_3\cdot \overrightarrow{O_3G_3}}{m_1+m_2+m_3}
$$
%\end{texteCache}
\item On peut exprimer $\overrightarrow{O_3G_1}$ et $\overrightarrow{O_3G_2}$ dans la base $b_3=\triplet{\overrightarrow{x}_3}{\overrightarrow{y}_3}{\overrightarrow{z}_{0,3}}$ :
%\begin{texteCache}
\begin{align*}
\overrightarrow{O_3G_1}=r\cdot \cos\Phi_1\cdot \overrightarrow{x}_3+r\cdot \sin\Phi_1\cdot \overrightarrow{x}_3+h\cdot \overrightarrow{z}_{0,3}\\
\\
\overrightarrow{O_3G_2}=r\cdot \cos\Phi_2\cdot \overrightarrow{x}_3+r\cdot \sin\Phi_2\cdot \overrightarrow{x}_3-h\cdot \overrightarrow{z}_{0,3}\\
\end{align*}
%\end{texteCache}
\item $\overrightarrow{O_3G}\cdot \overrightarrow{x}_3=0$ donne : 
%\begin{texteCache}
\begin{align*}
r\cdot \left(m_1\cdot \cos\Phi_1+m_1\cdot \cos\Phi_1 \right)=0\\
\end{align*}
%\end{texteCache}
\item $\overrightarrow{O_3G}\cdot \overrightarrow{y}_3=0$ donne : 
\begin{texteCache}
\begin{align*}
m_3\cdot b+r\cdot \left(m_1\cdot \sin\Phi_1+m_1\cdot \sin\Phi_1 \right)=0\\
\end{align*}
\end{texteCache}
\end{itemize}
...
\end{exemple}
\begin{exemple}[Détermination de l'équilibrage statique]
\subparagraph{}\textit{Traduire la relation donnée par l'équilibrage dynamique.}

\begin{itemize}
\item L'opérateur d'inertie de $E$ en $O_3$ doit avoir la direction $\couple{O_3}{\overrightarrow{z}_{0,3}}$  comme principale d'inertie d'où $D=E=0$.
\item On obtient alors une relation avec l'expression de $D$ : 
%\begin{texteCache}
\begin{align*}
D=D_3+m_1\cdot y_1\cdot z_1+ m_2\cdot y_2\cdot z_2=D_3+m_1\cdot r\cdot \sin\Phi_1\cdot h-m_2\cdot r\cdot \sin\Phi_2\cdot h
\end{align*}
%\end{texteCache}
\item On obtient alors une relation avec l'expression de $E$ :
%\begin{texteCache}
\begin{align*}
E=E_3+m_1\cdot x_1\cdot z_1+m_2\cdot x_2\cdot z_2=E_3+m_1\cdot r\cdot \cos\Phi_1\cdot h-m_2\cdot r\cdot \cos\Phi_2\cdot h
\end{align*} 
%\end{texteCache}
\end{itemize}
...
En notant,
\begin{itemize}
\item $x_i$, $y_i$ et $z_i$ ($i$ allant de $1$ à $2$) les coordonnées des deux masselottes dans le repère $\quadruplet{O}{\overrightarrow{x}_s}{\overrightarrow{y}_s}{\overrightarrow{z}_s}$ lié à un solide $S$,
\item $r_s$ la distance entre le centre d'inertie de $S$ par rapport à $\couple{O}{\overrightarrow{z}_s}$, 
\item $D_s$, $E_s$ et $E_s$ les produits d'inertie du solide S dans la base $b_s=\triplet{\overrightarrow{x}_s}{\overrightarrow{y}_s}{\overrightarrow{z}_s}$
\end{itemize}
l'écriture des condition d'équilibrage d'un solide en rotation autour d'un axe $\couple{O}{\overrightarrow{z}_s}$ donne : 

\begin{tabular}{|p{6.5cm}|p{6.5cm}|}
\hline 
\textbf{Centre d'inertie $G$ sur l'axe de rotation} & \textbf{Axe de rotation $\couple{O}{\overrightarrow{z}}$ axe principal d'inertie du solide $S$} \\ 
\hline 
$
...
&
$
D=D_s+m_1\cdot y_1\cdot z_1+ m_2\cdot y_2\cdot z_2
$
\\ 
$
m_3\cdot r_s+m_1\cdot y_1+m_2\cdot y_2=0
$
&
$
E=E_s+m_1\cdot x_1\cdot z_1+m_2\cdot x_2\cdot z_2
$\\
\hline 
\end{tabular} 
...
\subsection{Mise en oeuvre : machine d'équilibrage industriel}

\begin{propriete}[Mise en oeuvre de l'équilibrage dynamique]
...
\begin{center}
\includegraphics[width=0.4\textwidth]{images/machine_equilibrage.jpeg}
\end{center}
\end{propriete}
