...
\subparagraph{}
\textit{Déterminer le nombre de degrés de liberté et le nombre d’actionneurs du modèle proposé figure 9****. En
...
\subparagraph{}
\textit{Déterminer littéralement, dans la base $\mathcal{B}_2$ et au point $G_3$, la vitesse $\vectv{G}{3}{0}$ puis le torseur dynamique $\torseurdyn{D}{3}{0}$ de l’ensemble \{conteneur + spreader\} (3) dans son mouvement par rapport au repère galiléen $\mathcal{R}_0$.}
\ifprof
\begin{corrige}
\end{corrige}
\else
...
de résultante reliant les paramètres $\theta(t)$, $\beta(t)$ et $y_{ch}(t)$ et sans inconnue de liaison.}
\ifprof
\begin{corrige}
\end{corrige}
\else
