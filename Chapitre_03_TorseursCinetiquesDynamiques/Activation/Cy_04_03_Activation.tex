...
\def\xxtitreexo{Activation -- Éolienne}%Motorisation du moteur Haibike}
...
\begin{center}
\begin{tabular}{cc}
\includegraphics[width=0.35\linewidth]{images/eolienne_bipale.jpeg}
&
\includegraphics[width=0.55\linewidth]{images/eolienne.pdf}
\end{tabular}
\end{center}
...
%
%\begin{center}
%\includegraphics[width=\linewidth]{images/req_eolienne.pdf}
%\end{center}
...
\subparagraph{} \textit{Tracer le schéma de structure de l'éolienne.}
\ifprof
\begin{corrige}
...
C\cdot \dot{\alpha}\cdot \cos\beta+M\cdot a^2\cdot \dot{\alpha}\cos\beta\\
\end{array}
\right)_{\base{\overrightarrow{x}_2}{\overrightarrow{y}_2}{\overrightarrow{z}_2}}
\cdot
\left(
\begin{array}{c}
0\\
...
\subparagraph{} \textit{Déterminer la projection du moment dynamique de $3/0$ selon $\overrightarrow{z}_0$ : $\overrightarrow{z}_0\cdot \vectmc{K}{3}{0}$.}
...
\includegraphics[width=3cm]{images/figuregeom}
\end{center}
\end{minipage}
\begin{minipage}{0.5\textwidth}
\begin{align*}
...
\subparagraph{} \textit{Dans le cas d'une vitesse de rotation de l'hélice \textbf{2} ($\dot{\beta}$) constante et dans le cas où l'angle $\alpha$ est constant (pas de changement d'orientation de l'éolienne) déterminer l'expriment du couple $C_m$ que devrait fournir un moteur placé dans le mat (entre \textbf{0} et \textbf{1}) pour << contrer >> les effets dynamiques du balourd.}
